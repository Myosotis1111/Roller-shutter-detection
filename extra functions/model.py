...
def img_calibration(original_image_path, position):
    # open and read the image
    img = cv2.imread(original_image_path)

    # Open image files and read metadata
    ...
    # Initialize the corrective transformation mapping
    new_camera_matrix, roi = cv2.getOptimalNewCameraMatrix(camera_matrix, distortion_coefficients, image_size, 0,
                                                           image_size)
    ...
    # Load the image and perform distortion correction
...
class Model:
    ...
    def __init__(self):
        ...
        self.ms = MySignals()
        self.mode = "GUI"  # mode = "GUI" (default) or "test", the former updates GUI and the latter does not.
        self.txt_mode = "independent"  # txt_mode = "fixed" or "independent" (default)
        self.cam_dingcam02_txt_path = None  # the path of text file including camera position for camera position 1.
        self.cam_dingcam03_txt_path = None  # the path of text file including camera position for camera position 2.
        self.iou_thres = None
    ...
    def upload(self):
        ...
        if fileNames:
            ...
            self.uploadPreprocessing(fileNames)

    def uploadPreprocessing(self, filenames):
        ...
        for i, fileName in enumerate(filenames):
            ...
            #  transform the upload images into undistorted images
            if "undistorted" not in basename:
                if "dingcam02" in basename:
                    im = img_calibration(save_path, "dingcam02")
                elif "dingcam03" in basename:
                    im = img_calibration(save_path, "dingcam03")
                else:
                    im = cv2.imread(save_path)
                if self.mode == "GUI":
                    save_path = osp.join("images/tmp", f"tmp_upload_{i}_undistorted." + suffix)
                if self.mode == "test":
                    save_path = osp.join("evaluation/tmp", f"tmp_upload_{i}_undistorted." + suffix)
            ...
            if self.txt_mode == "fixed":
                # Read the labeled txt file, its path is fixed and can be configured in the beginning view.py
                if "dingcam02" in basename:
                    txt_path = self.cam_dingcam02_txt_path
                if "dingcam03" in basename:
                    txt_path = self.cam_dingcam03_txt_path
